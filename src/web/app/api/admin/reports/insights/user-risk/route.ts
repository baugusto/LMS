...
import { getUserRiskInsights } from "@/server/lms/services/reports.insights.service"
...
export async function GET(req: NextRequest) {
  try {
    ...
    const insights = await getUserRiskInsights(parsed.data)
    return NextResponse.json(insights)
  } catch (err) {
    console.error("Erro ao gerar insights de risco", err)
