...
import { CompanyProgressRow } from "@/server/lms/services/reports.service"

export function CompanyProgressGrid({ rows }: { rows: CompanyProgressRow[] }) {
      ...
      <div className="overflow-x-auto">
        <div className="min-w-[720px] space-y-2">
          <div className="grid grid-cols-[2fr_1fr_1fr_1fr_80px] text-xs text-muted-foreground px-2">
            <span>Empresa</span>
            <span title="Quantidade de perfis de parceiro associados aos usuários.">Perfis</span>
            <span title="Total de usuários vinculados à empresa.">Usuários</span>
            <span title="Média de progresso dos usuários (0% para quem não iniciou).">Progresso médio</span>
            <span>Ações</span>
          </div>
          <div className="space-y-2">
            {rows.map((row) => {
              const isOpen = open === row.companyId
              return (
                <Card key={row.companyId} className="shadow-none border border-brand-border">
                  <CardContent className="p-3 space-y-2">
                    <div className="grid grid-cols-[2fr_1fr_1fr_1fr_80px] items-center gap-2">
                      <div className="flex flex-col">
                        <span className="text-sm font-semibold">{row.companyName}</span>
                        <span className="text-[12px] text-muted-foreground">{row.partnerProfileNames.join(", ") || "-"}</span>
                      </div>
                      <div className="text-sm">{row.partnerProfileNames.length || "-"}</div>
                      ...
                      <button
                        ...
                        onClick={() => setOpen(isOpen ? null : row.companyId)}
                      >
